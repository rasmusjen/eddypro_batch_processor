...
|:---|:---|:---|
| **CPU Bound** | High CPU utilization sustained over time. | 🔴 **Red**: > 90% avg utilization<br>🟡 **Yellow**: 70-90% avg utilization<br>🟢 **Green**: < 70% avg utilization |
| **Memory Bound** | High RAM usage close to system limits, or excessive swapping. | 🔴 **Red**: > 85% of physical RAM<br>🟡 **Yellow**: 70-85% of physical RAM<br>🟢 **Green**: < 70% of physical RAM |
| **I/O Bound (Throughput)** | Low CPU usage paired with high Disk MB/s. | 🔴 **Red**: CPU < 20% AND Disk I/O > 80% of drive capability*<br>🟡 **Yellow**: CPU < 40% AND moderate I/O<br>🟢 **Green**: I/O does not correlate with low CPU |
| **I/O Bound (Latency/IOPS)** | Low CPU usage, low throughput, but high IOPS (many small files). | 🔴 **Red**: CPU < 20% AND IOPS > 1000 (typical HDD limit) or High Wait Time<br>🟡 **Yellow**: CPU < 40% AND moderate IOPS |
...
|:---|:---:|:---:|:---|
| **Processor (CPU)** | 🔴 | **YES** | **High Saturation.** Average usage 94%. We recommend reducing `max_processes` or moving to a larger machine. |
| **Memory (RAM)** | 🟢 | No | Peak usage 4.2GB (13% of System RAM). Plenty of headroom. |
| **Storage (I/O)** | 🟡 | Potential | Moderate read latency detected during "Raw Data Processing" phase. |
```
