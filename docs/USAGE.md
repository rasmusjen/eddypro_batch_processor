...
- `--version`: Print the installed `eddypro-batch` version and exit
- `--help`, `-h`: Show help message and exit

**Logging to file:** Configure `log_file` in your config to write logs to a file
in addition to the terminal. Use `log_max_bytes` and `log_backup_count` to
enable rotation, and `log_eddypro_output` to include EddyPro stdout/stderr. See
...
| `--monitor` | flag | Enable performance monitoring (default) |
| `--no-monitor` | flag | Disable performance monitoring (no metrics files; ignores `--metrics-interval`) |
...
**Reporting:** `scenarios` generates a per-scenario HTML report at
`{output_dir}/{scenario_suffix}/reports/run_report.html` for each scenario,
plus one aggregate comparison report and `run_manifest.json` under
`reports_dir`. See [REPORTING.md](REPORTING.md).
...
[PASS] Config Structure: OK
[PASS] Config Sanity: OK
[FAIL] Paths: 1 error(s)
...
**Note:** This command reads the most recent `run_manifest.json` and prints a
summary table for scenarios, duration, and success status.
...
For a complete, real-world multi-year walkthrough (config file and pure-CLI
forms, choosing `max_processes`, reading the bottleneck report), see
...
For a complete worked example of processing several years with the same
settings (config file and pure-CLI forms), see [MULTI_YEAR_RUNS.md](MULTI_YEAR_RUNS.md).
